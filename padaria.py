...
from PyQt5.QtWidgets import QApplication,QWidget,QHBoxLayout,QVBoxLayout,QLabel,QTableWidget,QTableWidgetItem,QLineEdit,QComboBox,QMessageBox,QInputDialog
...
class caixa2(QWidget):
    def __init__(self):
        ...
        self.imagem_label.setPixmap(QPixmap(".venv/padoka.jpg"))
    ...
    def capturaTecla(self, e):
        if(e.key()==Qt.Key_F2):
            ...
            self.total = self.total + float(self.preco_edit.text())

            self.totalapagar_edit.setText(str(self.total))
            ...
            if op == QMessageBox.Yes:
                ...
                if ok :
                    #vamos criar uma lista(array) para guardar
                    #todos os dados da tabela para criar uma
                    #nota fiscal
                    dados = []
